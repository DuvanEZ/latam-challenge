...
def main():
    ...
    question = st.sidebar.radio("Choose a question to view", ("Question 1","Question 2", "Question 3"))

    st.title("Twitter Data Analysis")

    # Create separate containers for each table
    container_q1_time = st.empty()
    container_q1_memory = st.empty()
    ...
    if question == "Question 1":
        with container_q1_time.container():
            st.header("Question 1")
            st.subheader("Execution Time")
            fig_q1_time = go.Figure(data=[go.Table(
                header=dict(values=list(q1time_df.columns),
                            fill_color='paleturquoise',
                            align='left'),
                cells=dict(values=[q1time_df[col] for col in q1time_df.columns],
                           fill_color='lavender',
                           align='left'))
            ])
            st.plotly_chart(fig_q1_time)

        with container_q1_memory.container():
            st.subheader("Memory Usage")
            fig_q1_memory = go.Figure(data=[go.Table(
                header=dict(values=list(q1memory_df.columns),
                            fill_color='paleturquoise',
                            align='left'),
                cells=dict(values=[q1memory_df[col] for col in q1memory_df.columns],
                           fill_color='lavender',
                           align='left'))
            ])
            st.plotly_chart(fig_q1_memory)

        # Clear the containers for question 2 and 3
